									...
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
											...
											<span class="badge badge-pill badge-warning mr-auto my-auto float-left">
                                                <a href="{{ Route('markread') }}">تعين قراءة الكل</a>
                                            </span>
										</div>
										<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">عدد الأشعارات الغير مقروءة : {{ auth()->user()->unreadNotifications->count() }}</p>
									</div>
									<div class="main-notification-list Notification-scroll">
									@if(auth()->user()->unreadNotifications->count() > 0)
										@foreach(auth()->user()->unreadNotifications as $notification)
										<a class="d-flex p-3 border-bottom" href="{{ Route('markThisRead' , $notification->id)  }}">
										...
										@endforeach
									@else
										<h4 class="text-center">لا يوجد اشعارات جديدة</h4>
									@endif
									</div>
								</div>
							</div>
